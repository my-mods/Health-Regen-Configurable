...
The reference calculation layout was established on Dawnwalker 1.0.5 / Steam build 25232147, EXE SHA-256 `cb9b7d7bd88a6754c0a9c08318aa64d5013ddfd92d5badcae84e1b4ea980dcfc`. This hash is provenance only: the helper does not read or hash the game executable file. Before exposing its Lua API, `GameCode.hpp` validates the loaded base table at RVA `0x76c77f0` (92 entries, compared relative to the actual load address) and the complete four-byte zero-return calculation at RVA `0x1271ab0`. All ranges must be readable within the game image and the calculation must be executable. Unrelated file differences and storefront identity are not rejection criteria. A changed required table, code or address stops initialization and logs its RVA. The separate loader hash remains required for the pinned UE4SS C++ ABI.

Setup also rejects another CDO table or calculation slot and probes both actual virtual calls before enabling the effect. Function signatures use the Windows x64 ABI: float result, calculation object and a const effect-spec pointer. The spec is not read or retained. This scoped validation does not automatically port changed game layouts.
